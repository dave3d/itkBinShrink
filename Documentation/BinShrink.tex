...
filter has been optimized it efficiently access the input image thereby
...
Our new \textif{BinShrink} filter reduces the resolution of an input
...
The comparison of the effect filtering with Gaussian version a box
(See Figure \ref{fig:BinShrinkComparison} verses
\ref{fig:GaussianShrinkComparison}}) is quite revealing of the
aliasing which can be caused by using a box filter. This is most
...
filters access the number number of input pixels and output pixels to
perform the same computation. The difference it the type of iterator
used and the order in which the images are accessed. The differences
in the iterator should be accounted for in with the time difference
with a shrink factor of 2. However, the large increase in the
\textit{BinShrink2} execution times can not be explained by the
differents in the iterator operation costs. Specifically our
